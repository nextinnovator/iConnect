    ...
    <nav ref="menuIcon" class="navIcon" :style="iconDirection">
      <a href="javascript:void(0)" @click="toggleMenu()" data-test-ref="navMenuLink">
        <svg width="30" height="30">
          <path d="M0,5 30,5" :stroke="styles['menu-icon-color']" stroke-width="5"></path>
          <path d="M0,14 30,14" :stroke="styles['menu-icon-color']" stroke-width="5"></path>
          <path d="M0,23 30,23" :stroke="styles['menu-icon-color']" stroke-width="5"></path>
        </svg>
      </a>
    </nav>
  </div>
...
<script>
import styles from "assets/color.scss";
import utilities from "services/auth/utilities";
export default {
  name: "slider",
  props: {
    width: {
  ...
  methods: {
    openMenu() {
      if (this.opacity) {
        document.body.style.backgroundColor = utilities.hexToRGB(
